import { checkHasAmazonStore, checkHasPharmaceuticalExport, getGstDetails } from "../../util/functions";
...
interface Props {
  ...
  onIndustryChange: (value: string, filteredOption: Option[]) => void;
}

interface McaAutoFetchInfoProps {
  className?: string;
}

const McaAutoFetchInfo = (props: McaAutoFetchInfoProps) => {
  const { theme } = useContext(AppContext);
  return (
    <div className={classNames("flex_row_item_center ", props.className)}>
      <InfoIcon
        height={16}
        width={16}
      ...
      />
      <Typography text={Locale.mcaDocsAutoFetch} size={TYPOGRAPHY_SIZES.SMALL} textClasses={"!text-black-500"} />
    </div>
  );
};

const CompanyPANDetailsForm = (props: Props) => {
  const { theme } = useContext(AppContext);
  const {
    exporterData = {},
    industryList = [],
  ...
  const entityName = isLlp ? Locale.llp : Locale.company;
  const [isConfirmPopupOpen, openConfirmPopup] = useState(false);
  const analytics = useAnalytics();
  const { isMobile } = useMobileDetect();
  const isBusinessNameMissing = !businessLegalName;
  const isCinMissing = !cin;
  const showDateOfIncorporation = !exporterData.dateOfIncorporation;
    ...
    try {
      const response = await beCall({
        ...
        body: {
          ...
          ...(isBusinessNameMissing ? { businessLegalName: values.businessLegalName } : {}),
          ...(!isPartnership && isCinMissing ? { cin: values.cin } : {}),
          ...
          systemSelectedIndustryOption?.riskCategory === "MEDIUM"
            ...
            : {}),
          // Handle Amazon URL for industry questions about Amazon
          ...(hasAmazonQuestion
            ? {
                hasAmazonStore: true,
      ...
      if (response.success) {
        ...
        isPartnership && refetchCompanyPan();
        refetchUserState();
      } else {
        throw response;
      }
  ...
  const validateForm = (values: any) => {
    ...
    const errors: Error = {};
    if (!isPartnership && !values.cin) {
      errors.cin = `${isLlp ? "LLP" : "C"}IN required`;
    }
    ...
    if (!values.companyShortName) {
      errors.companyShortName = isPartnership
        ? Locale.shortNameRequiredError
        : `${isLlp ? "LLP" : "Company"} nickname for communications is required`;
    }
    ...
    if (!values.companyAddress || !values.companyAddress.trim()) {
      errors.companyAddress = `Billing address is required`;
    }
    if (selectedIndustryOption.riskCategory === INDUSTRY_CATEGORY.OTHERS && !values.industryDescription) {
    ...
    <div className={classNames("flex flex-col flex-1")}>
      ...
      >
        {({ handleSubmit, errors, values, submitError, form, submitFailed }) => {
          ...
          const renderDateOfIncorporationField = () => (
            <Field name={"dateOfIncorporation"}>
              {(props) => (
                <DateSelector
                  label={Locale.dateOfIncorporation}
                  containerClass={"flex-1"}
                  selectedDate={props.input.value ? props.input.value.split("-").reverse().join("/") : undefined}
          ...
          return (
            <form onSubmit={handleSubmit} className={"md:overflow-y-visible pb-44 md:pb-0"}>
              <FormSpy
              ...
              <div className={"flex-1 flex flex-col md:flex-row mb-6"}>
                <Field name={"businessLegalName"}>
                  {(props) => (
                    <TextInput
                      isDisabled={!isBusinessNameMissing}
                      label={
                        isPartnership ? Locale.businessNameHeader : Locale.companyName.replace("entity", entityName)
                      }
                      inputClass={"w-full md:w-1/2 md:mr-4"}
                ...
                <div className={"mt-4 md:mt-0 w-full md:w-1/2"}>
                  {isPartnership ? (
                    ...
                    <Field name={"cin"}>
                      {(props) => (
                        <TextInput
                          isDisabled={!isCinMissing}
                          label={isLlp ? Locale.llpIN : Locale.CINnumber}
                          inputClass={"w-full"}
                          value={props.input.value}
                          onChange={(value: string) => props.input.onChange(value?.toUpperCase())}
                          isError={props.meta.submitFailed && props.meta.error}
                          footerText={props.meta.submitFailed && errors?.cin}
                        />
                      )}
                    </Field>
                  )}
              ...
              {!isPartnership && (
                <div className={"flex-1 flex flex-col md:flex-row mb-6"}>
                  <Field name={"companyShortName"}>
                    {(props) => (
                      <TextInput
                        label={Locale.compShortName.replace("entity", entityName)}
                        isLabelRequired={true}
                        inputClass={classNames("w-full md:w-1/2", { "md:mr-4": gstOptions.length > 0 })}
                        value={props.input.value}
                        onChange={props.input.onChange}
                        onBlur={() => {
                          if (props.input.value !== correspondentName) {
                  ...
                  {gstOptions.length === 0 ? null : gstOptions.length === 1 ? (
                    <TextInput
                      ...
                      inputClass={"flex-1 mt-4 md:mt-0"}
                    ...
                    <Field name={"primaryGstId"}>
                      {(props) => (
                        <Dropdown
                          ...
                          containerClass={"flex-1 mt-4 md:mt-0"}
                          options={gstOptions}
                          onSelect={(gstId) => {
                ...
                <Field name={"industryId"}>
                  {(props) => (
                    <SearchDropdown
                      ...
                      isLabelRequired={true}
                      className={"flex-1 md:mr-4 mb-4 md:mb-0"}
                      selectedValue={props.input.value}
                      isError={props.meta.submitFailed && props.meta.error}
              ...
              {!isPartnership || gstOptions.length === 0 ? null : (
                <div className={"flex-1 mr-4 w-full md:w-1/2 mb-6"}>
                  {gstOptions.length === 1 ? (
                    <TextInput
                      isDisabled={true}
                      label={Locale.gstinNumber}
              ...
              <Field name={"companyAddress"}>
                {(props) => (
                  <TextInput
                    ...
                    isDisabled={false}
                    label={Locale.companyAddress}
                    isLabelRequired={true}
                    onChange={props.input.onChange}
                    onBlur={() => {
                      if (props.input.value !== communicationAddress) {
                        analytics.trackAsync(Events.BILLING_ADDRESS_EDITED, { entity_type: businessType });
                      }
                      props.input.onBlur();
                    }}
                    type={"textarea"}
                    inputClass={"mb-6 mt-4 md:mt-0"}
                    inputProps={{
                      rows: 3,
                    }}
                    value={props.input.value}
                    isError={props.meta.submitFailed && props.meta.error}
                    footerText={props.meta.submitFailed && errors?.companyAddress}
                  />
                )}
              </Field>
              {showDateOfIncorporation ? (
                <div className={"flex flex-1 flex-col md:flex-row mb-6"}>{renderDateOfIncorporationField()}</div>
              ) : null}
              <McaAutoFetchInfo className={"hide_for_desktop_flex mt-6"} />
              <div
                className={
              ...
              <div className={"hide_for_mob flex flex-col"}>
                ...
                <McaAutoFetchInfo className={"mt-6"} />
              </div>
            </form>
          );
        }}
...
export default CompanyPANDetailsForm;
