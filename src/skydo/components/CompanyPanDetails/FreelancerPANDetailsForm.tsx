...
import {
  ...
  BUSSINESS_TYPES,
  EcommerceOptions,
  FREELANCER_MONTHLY_INCOME_OPTIONS,
...
const FreelancerPANDetailsForm = (props: Props) => {
  ...
  const isFullNameMissing = !exporterUserFullName;
  ...
  const validateForm = (values: any) => {
    ...
    if (!isIndividualBusiness && !values.primaryGstId) {
      errors.primaryGstId = `GSTIN is required`;
    }
    if (!gstOptions?.length && !values.monthlyRevenue) {
      errors.monthlyRevenue = Locale.monthlyRevenueRequiredError;
    }
    const isDescriptionValid =
      !!values.businessActivityDescription &&
      values.businessActivityDescription.length >= MIN_BUSINESS_ACTIVITY_DESC_LEN;
                  ...
                  <Field name={"averageTransaction"}>
                    {(props) => (
                      <Dropdown
                        ...
                        containerClass={"mb-6 md:max-w-xs"}
                  ...
                  {gstOptions.length === 0 ? (
                    <Field name={"monthlyRevenue"}>
                      {(props) => (
                        <div className={"flex flex-col mb-6"}>
                          <Typography
                            text={`${Locale.monthlyIncomeFromClients} `}
                            type={TYPOGRAPHY_TYPES.LABEL}
                            size={TYPOGRAPHY_SIZES.X_SMALL}
                            textClasses={"mb-2"}
                          >
                          ...
                          <div className={"flex flex-row space-x-3"}>
                            {FREELANCER_MONTHLY_INCOME_OPTIONS.map((option) => (
                              <Button
                                key={option.value}
                                nativeType={"button"}
                                title={option.label}
                                type={BUTTON_TYPES.SECONDARY}
                                buttonClass={classNames("flex-1 justify-center", {
                                  "!bg-blue-50 !border-blue-400": props.input.value === option.value,
                                })}
                                textClasses={classNames(
                                  "!text-labelxsmall !font-semibold !leading-4 whitespace-nowrap",
                                  props.input.value === option.value ? "!text-blue-400" : "!text-black-600"
                                )}
                                onButtonClick={() => {
                                  analytics.trackAsync(Events.MONTHLY_INCOME_SELECTED, { selected_band: option.value });
                                  props.input.onChange(option.value);
                                }}
                              />
                            ))}
                          </div>
                          {props.meta.submitFailed && errors?.monthlyRevenue ? (
                            <Typography
                              text={errors.monthlyRevenue}
                              size={TYPOGRAPHY_SIZES.SMALL}
                              textClasses={"!text-red-400 mt-2"}
                            />
                          ) : null}
...
export default FreelancerPANDetailsForm;
